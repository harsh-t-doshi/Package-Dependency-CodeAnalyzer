...
namespace Lexer
{
    ...
    // - Factory class creates the objects of semiexpression and toker.
    ...
    //-------<all the tokens are collected in the list>------------------
    public class Semi : ITokenCollection
    {
        ...
        public bool hasTerminator()   //----< does this semi-expression contain a terminator? >--------
        {
            if (toks.Count <= 0)
                return false;
        ...
        public void show()   //----< display contents of semi-expression on console >---------
        {
            Console.Write("\n-- ");
            foreach (Token tok in toks)
            {
    ...
    class TestSemi
    {
        ...
        private static void checkSemiTest(ITokenCollection test)
        {
